...
class TravelAdvisor:
    ...
    MESSAGE_READY = "Du kannst losfahren"
    MESSAGE_CURRENT_TRAVEL = "Deine momentane Reisezeit beträgt: "
    ...
    def handle_command(self,command, user_id, channel):
        """
            Receives commands directed at the bot and determines if they
            are valid commands. If so, then acts on the commands. If not,
            returns back what it needs for clarification.
        """
        travel_command = "Wann soll ich losfahren?"
        response ="Tut mir leid, das verstehe ich noch nicht =("

        intent = self.get_watson_intent(command)
        if intent == "when_should_I_leave":
    ...
    def ask_for_target_duration(self, channel):
        response = "Ab wieviel Minuten oder weniger soll ich dir bescheid geben?"
        self.send_message(response, channel)

    def ask_for_destination(self, channel):
        response = "Danke. Wohin willst du fahren?"
        self.send_message(response, channel)

    def ask_for_origin(self,channel):
        response = "Ich habe erkannt, dass du wissen willst wann du losfahren sollst. Von wo willst du losfahren?"
        self.send_message(response, channel)

    def already_existing_request(self, user, channel):
        response = "Du hast bereits einen bestehenden Auftrag. {}{}".format(self.MESSAGE_CURRENT_TRAVEL,user.travel_request.duration_in_traffic['text'])
        self.send_message(response, channel)

    def handle_travel_request(self, request, channel):
        request.check_current_travel(self.gmaps)

        message = "{}{}".format(self.MESSAGE_CURRENT_TRAVEL,request.duration_in_traffic['text'])
        self.send_message(message,channel)

        message = "Ich gebe dir bescheid sobald die Reisezeit weniger als {} Minuten beträgt.".format(int(request.target_duration/60))
        self.send_message(message, channel)
    ...
    def check_travel_request(self, request):
        ...
        if request.counter > self.MAX_TRAVEL_CHECKS:
            ...
            if request.duration_in_traffic['value'] <= request.target_duration:
                message = "{} nach {}".format(self.MESSAGE_READY,request.destination)
    ...
    if bot.slack_client.rtm_connect():
        ...
        while True:
            ...
            if command and channel and user:
                bot.handle_command(command, user, channel)
